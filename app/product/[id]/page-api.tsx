...
import { ChevronLeft, ShoppingBag, CreditCard, AlertCircle, Loader2 } from "lucide-react"
...
import { adaptProductDtoToProduct } from "@/lib/api-adapter"
...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  // Usar el hook para obtener el producto de la API
  const { product: productDto, loading, error } = useProduct(params.id)

  // Convertir el DTO a nuestro modelo de producto
  const product = productDto ? adaptProductDtoToProduct(productDto) : null

  // Si hay un error o el producto no existe, redirigir a la página de productos
  useEffect(() => {
