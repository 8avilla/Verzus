...
export default function BoldPaymentButton({
  ...
  useEffect(() => {
    const renderButton = async () => {
      if (!isScriptLoaded || !buttonContainerRef.current || isSubmitting) return
      ...
      try {
        ...
        if (scriptRef.current && buttonContainerRef.current.contains(scriptRef.current)) {
          buttonContainerRef.current.removeChild(scriptRef.current)
        }
